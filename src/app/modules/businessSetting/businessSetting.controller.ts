...
import { businessSettingsService } from "./businessSetting.service";

// Get the privacy policy
const getPrivacyPolicy = async (req: Request, res: Response) => {
    try {
        const policy = await businessSettingsService.getSettingsByKey({key: "privacy_policy"});
...
const getTermConditions = async (req: Request, res: Response) => {
    try {
        const policy = await businessSettingsService.getSettingsByKey({key: "term_condition"});
...
const getCookiePolicy = async (req: Request, res: Response) => {
    try {
        const policy = await businessSettingsService.getSettingsByKey({key: "cookie_policy"});
        ...
        console.error("Error retrieving cookie policy:", error.message);
        sendResponse(res, {
            statusCode: httpStatus.INTERNAL_SERVER_ERROR,
            success: false,
...
const getAboutUs = async (req: Request, res: Response) => {
    try {
        const policy = await businessSettingsService.getSettingsByKey({key: "about_us"});
...
const updateSettingsByKey = async (req: Request, res: Response) => {
    try {
        ...
        const updatedPolicy = await businessSettingsService.updateSettingsByKey(key, content);
